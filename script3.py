...
# -*- coding: utf-8 -*-
"""
Hindi Text Extractor using Dolphin Mistral 24B Venice Edition on Apple Silicon (MLX)
"""
...
def run_dolphin_infer(model_id: str, prompt: str, max_tokens: int = 1024, temp: float = 0.2) -> str:
...
def extract(text: str, model_id: str) -> Dict[str, Any]:
    ...
    raw = run_dolphin_infer(model_id, prompt)
...
def main():
    parser = argparse.ArgumentParser(
        description="Hindi Text -> Structured Extraction via Dolphin Mistral 24B Venice Edition")
    ...
    parser.add_argument("--model", type=str, default="mlx-community/Dolphin-Mistral-24B-Venice-Edition-4bit",
                        help="HF repo id of an MLX-compatible Dolphin Instruct model")
...
# python script3.py --file input.txt --out result.json
